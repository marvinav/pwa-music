export class IcyCastReader {
    ...
    private read = async () => {
        ...
        try {
            ...
            const icyHeaderLengh = mergedArray[this.metaInt] ?? 0;
            // If current buffer doesn`t contain all metadata when skip
            if (this.length < this.metaInt + icyHeaderLengh * 16 + 1) {
                if (!done) {
                    this.read();
                } else {
                    let offset = 0;
            ...
            if (icyHeaderLengh > 0) {
                this._icyHeader = String.fromCharCode.apply(
                    null,
                    mergedArray.slice(this.metaInt + 1, this.metaInt + 1 + icyHeaderLengh * 16),
                );
            }
            ...
            let newArray = mergedArray.slice(this.metaInt + 1 + icyHeaderLengh * 16);
    ...
    private done = () => {
        this.onDone && this.onDone({ recieved: this.recieved, send: this.send });
        this._state = 'stopped';
    };
